...
// Variáveis de escopo
let chatSocket = null;
// Variável alterada de 'canalId' para 'canalSlug'
let canalSlug = null;
let currentUserId = null; // Variável corrigida para refletir o nome do contexto Django
...
// Elemento para a lista de membros na sidebar
let membersListWrapper = null;
// Elementos da sidebar e toggle
...
function onWsMessage(e) {
    ...
    if (data.type === 'chat_message') {
        ...
        // Usa currentUserId (variável corrigida) para determinar se a mensagem é do usuário
...
function createMessageElement(messageData, isUserMessage) {
    ...
    // MANTENDO APENAS A CLASSE ESSENCIAL
    ...
    // CORREÇÃO ROBUSTA:
    // Verifica se a string parece já formatada (DM), caso contrário, formata.
    if (messageData.timestamp && messageData.timestamp.length <= 6 && messageData.timestamp.includes(':')) {
...
function initChat() {
    ...
    // 2. Garante que os elementos CRUCIAIS DE MENSAGEM existam
    if (!chatLogWrapper || !chatLog || !messageInput || !messageSubmit) {
        console.error("ERRO CRÍTICO: Elementos DOM essenciais do chat (log ou input) não encontrados.");
        if (messageInput) messageInput.placeholder = "ERRO: Inicialização do Chat Falhou.";
        // Parar se os elementos de MENSAGEM não existirem.
        return;
    }

    // AVISO: Se algum elemento de sidebar não foi encontrado, a funcionalidade
    // de sidebar/toggle pode falhar, mas o chat principal pode continuar.
    // Garante que o chat principal funcione mesmo sem a sidebar
    if (!membersListWrapper || !sidebarElement || !sidebarToggleButton || !mainContent) {
        console.warn("AVISO: Elementos opcionais da Sidebar não foram encontrados. O chat continuará, mas a sidebar pode não funcionar.");
    }


    // 3. LÓGICA DE INICIALIZAÇÃO DO WEBSOCKET
    try {
        // CORREÇÃO: Altera de 'canal-id' para 'canal-slug'
        const canalSlugElement = document.getElementById('canal-slug');
        const userIdElement = document.getElementById('user-id'); // Este ID ainda é usado no HTML

        // Lança erro se elementos não forem encontrados
        if (!canalSlugElement || !userIdElement) {
            throw new Error("Elementos 'canal-slug' ou 'user-id' não encontrados no template HTML.");
        }

        // Leitura e Conversão Segura
        canalSlug = canalSlugElement.textContent.trim();
        // Lendo o valor que agora é '{{ current_user_id }}' no HTML
        currentUserId = parseInt(userIdElement.textContent.trim());

        // Validação para slug (string não vazia)
        if (!canalSlug) {
            throw new Error("canalSlug é inválido. Valor lido: '" + canalSlugElement.textContent.trim() + "'");
        }
        ...
        // Cria um objeto dummy/mock para evitar erros posteriores de 'chatSocket is null'
        chatSocket = {
        ...
        if (messageInput) messageInput.placeholder = "ERRO: Inicialização do Chat Falhou. Consulte o Console (F12).";
        // REMOÇÃO DA LINHA INCORRETA: if (messageInput) messageInput.disabled = true;
        if (messageSubmit) messageSubmit.disabled = true;
    }

    // 5. Binds de Eventos de Input
    if (messageInput) messageInput.oninput = toggleSubmitButton;

    if (messageInput) messageInput.onkeyup = function(e) {
        if (e.key === 'Enter' && !e.shiftKey) {
            e.preventDefault();
            if (messageSubmit && !messageSubmit.disabled) {
                sendMessage();
            }
        }
        toggleSubmitButton();
    ...
    if (messageSubmit) messageSubmit.onclick = sendMessage;

    // 6. BIND DE EVENTO DA SIDEBAR
    if (sidebarToggleButton) {
